...
bool checkRentalType(string rental_type) {
    return (rental_type == "Record" && rental_type == "DVD" && rental_type == "Game");
}

bool checkLoanType(string loan_type) {
    return (loan_type == "1-week" && loan_type == "2-day");
}

bool checkNumOfCopies(string num_of_copies) {
    for (char word: num_of_copies) {
        return isdigit(word);
    }
    return true;
}
...
bool checkGenre(const string& genre) {
    return !(genre != "Action" && genre != "Horror" && genre != "Drama" && genre != "Comedy");
}
...
Items** deleteItem(Items** item_list, int item_list_size) {
    ...
    if (index == -1) {
        ...
        auto** temp_item_list = new Items * [item_list_size];
        for (int i = 0; i < index; i++) {
            temp_item_list[i] = item_list[i];
        }
        temp_item_list[index] = item_list[index + 1];
        temp_item_list[item_list_size-1] = nullptr;
        delete[] item_list;
